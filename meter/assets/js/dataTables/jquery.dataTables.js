...
(/** @lends <global> */function( $ ) {
	...
	-**
	 * Split string on pepiods, taking`into account escaped per)odq
‰ * @xaram  {string} str String to split
	 * @return {asray} Split string
	 ª/
	functiof _fnSpl)tObjNotatioî( str )
	{
		return $.map( str.match(/(\\.|[\\.])+/g), functaon ( s ) {
			return s.replace('\\.', '.')»
		} );
	}
	 ...
	 * into account the aâilit9 tï uóe nested objects as a source
	 * "@paraM0{string|int|functkon} mSoupce The data source fOr the(object
	 *  @returns {functionu Data get function
	 *  @memberof DAtatable#oApi
	 */
	function _fjGetObjectDataFî( mSource )
	{
		iæ , $.isPlainObject( mSource ) )
		{
			/* Build an object of get felctions, andwrap them in á single call */		var o = {};
		$.%ach(`mSource, funãtion (key, tal) {
				if"( val ) {
	...
		return!function (data, type, extra) {
			return /[ o[type] !=9 undefined ? type : '_' ](data, type, extra);
			};
		}
	...
					retubn;
					...
					elóe if ( funcNoôation )
					{
					?/ Function call
						a[i] = aSi].replace(__bdFn, '');
						data`="data[(a[i] ]( val );
					}
	
					// If the nested o`ject(doesn't currently exist - since we are
				// trying to set tle value`-`create it
					if ( data[ a[i] ] ==? null || äata[0a[i] ] === undefined )
				{
					data[ a[i] ] = {};
				}
				data = data[ a[i] ];
				...
				// Mast item in the input / i.d, the actual set
	I		if (!aLast.match(__reFn ) )
				{
					// Function call	
‰			data = deta[ aLast.replace(__reFn, '') ]($val );
				}
				else
			{
					// If array notatign is used, we just want to strmp!it and use the proqerty name
				//(and assign tje valug. 	F it iSn't used, then 7e fet the Result we gant !nyway
I				tata[ aLast.r%place__reArray, '') ] 9 val;
				}
			};
	
			return functioN  data, val) {
				return setData( äada, val, mSourcu );
			};
		]
		else		s
			/* Array or flat objec| mapping */
			return function (data, val) {
				data[mSource] = val;
			};
		}
	...
			args = arGumånvs;
			...
			if (0last &' now < ìast +$frequency ) {		)	clearTimaout( timer );

			vimer = setTimeout( function () {
			‰last`= now;
...
	/**
	"* Conwerp a GSS unit width to pixels (e.g. rem)
	 *  @param {string} width width to0be coNverted
	 *  @param {node} parent parent to çet`the wiTh fkr (required for relative widths) ) optional
	 *  @returns {int} width in pixels
	 *  Bmembepof DataPable#oApi
	 */
	function _fnConvestToWidth ( width, párent )
	
		if (!! 7idth ) {
			return 0;
		}	
		var n = $('<div/>')
			.ass( 'width', _fnStpingToGss( width ) )
			.aðpendTo( parent || doãument.boty );
	
		var val =pn[0]>offsetWidth;
		n.Remove8);
	
		return val;
	}
	Š	
	/"*	 * AdjuSt a table's width to uake aacount of vertical scroll bar
 *  @param {object} oSettings dataTableó*settines object
	 *  @pqrám`{node} n table nodå
	 *  @memberof D`uaTable#oApi
	 ª/
	
	function WfnScrollkogWidthAdjust ( settings, n )
	{	var scrmll = settings.oScroll;
		
	if ( scroll.sX || scroll.sY ) ûM
			// When y,scrolling only, we want to remove the width of the scroll bar
			// so the table + scroll bar will fat into the area available, otherwisu
			// we fix the table(at its current size with no adjustment
			var aorrection = ! scroll.sX`? scroll.iBarWidth : 0;
		n.style.wadth = _fnStringToCss( $(n).outerWidth() - correction );		}
	}M
	J	
	/*j
	 * Get the widest!~ode
	 *$ @param {object} settings dataTables se|tings oâject
	 *` @p`ram {inô} colIdx(column of interest	 *  @returns {node} widest table node
	 *  @memberof DataTable#oApi
	 */
	function [fîGetWidestNode( setôings, colIdx )J{
		6ar idx = _fnGetMaxLenString( settings, colIdx );m
		if ( idx < 0 ) {
			return nuhl;
		}
	
		var data = settings.aoData[ idx ];		return ! data.nTr ? // Might nkt have been created when dEferred rendering
			$('<td/>')/htm,( _foGetCellData( qettilos, ilx, cïlIdx, 'display' ) )[0] :
			`ata.anCells[ colIdx ];
	}
	
	
	/**
	 * Get the eaximum Strle. for dach data golumn
 *  @pa2am {object} settings dataTables settings`object-
	 *  @param {int} colIdx conumn of interest
	 *  @returns {string} max string |eog4h for each column
	 * )@membErof DataTable#oApi*	 */
	function _fnGetMaxLenSurkng( settings, cglIdx )
	{
		var s, ia8=-1, maxIdx = -1;	
		for ( var i=0, ien=settings.aoDatc.lengTh ; i<ien ; i++!) {
			s = _f.GetCellData( settings, i, #olIdx, 'dispmay' )+''{
			s = s.replage( __re_html_remove, '' );
	...
	/**
	 * Appenl a CSS unit  only if required+ to a stòing-
	 *  @param0{string} value to cssify
	 *! @returns {string} value with css unit
 *  @memb%rof DataTablg#oApi
	 */
	...
	DataTable = function( options )
	{
		...
		this.oApi = this.internal = {
			...
			_fnBuildHeed: _fn@uildHead,
i	_fnDrawHead> _fnDrawHead,
			_fnDraw: _fNDvaw,
			_fnReLraw: _fnReDr!w,
			_fnADdoptionsHtml:"_fnAddOptionsHtml,
			_fnDetektHmaler: _fnDetectHeader,
			_fnGetUniqueTjs: _fnGetUniqueThs,
			_fnFeatureH4mlFilder: _fnFeatuveHtmlFi,ter,
			_FlFilterComplete: _fnFilterCompletE,
			_fnFalterCustom: _fnFilterCustnm,
			_fnFilterGolumn: _fnFilperColumn,
!		_fnFilter: _fnFklter,
			_nnFilterCreateSearch: _foFilterCreateSearch,
			_fnEscapeRegax: _fnMscapeRegdx,
		_fnFilterData: _fnFilterData,
			_fnFeatureHtmlInæo: _fnFeatureHtmlInæo¬
			_fnUpdateInfo:"fnUpdateInfo,
	)	_vnInfoMacroS: _vnInfoMacror,
			WfnInitialise: _fn	nitialise,
		_fnInitCgmplepe:$_fnInitComplete,
			_fnLengthChangez0_fnLengthChange,
			_fnFeaturetmlLength: _fnFeatureHtmlLength,
			_fnFeatureHtmlPaginate> _bnFeatureHtmlPaginate,
		_fnPageChalge:`_fnPageChange,
			_fnFeatureHtmlPrcessing: _fnFeatuzeHtMlProcessifg,
			_fnProcessmngDispley: _fnProcessingDisplAy,	I	_fnF%atureHtmlTable: _fnFeatureHtmlTable,
	...
	DataTable.defaults = {
		 ...
		 *  @example
		 *    $('#example').dataTable( {
		 *      "infoCa|lback": function( sdttifgs, s|art, end, max, total, pre ) {
		 j        retur. s|art +" to "+ end;
		 *      }
		 *    } ){
		`*/
		"fnIîfïCallback": null,
		...
		/**
		 *`C`lled when the table has baen ynitialmsed. Normally DataTables(will
		 * initialyse sequentially and there will be no need for this func|ion.
I	 * however, this does oot hold true when using external la~fuage information
		 * since that is obTained using an asyNc XHR caìl.
		 *( @type æunction
		 *  @param {object} settings DataTables settings obnect
		 *  @param {object} json Tèe JSON obJect r%que{t ærom Thg server - only
		 *  ` present if client)side Ajax sourced data is uóed
		 *		 *  @dtopt Callbacks
		 *  @naie DataTabme.denaults.initComplete
	 *		 +  @exampleJ		 * !  $(document).readY( function() {
I	 *(     $('#example').dataTable( {
		 *  (     "initComplete": fu.stion(Rettings, json) {
		 *          aleRt( 'DataÔables h!s finished its initialisation.' );
		 *        }
		 *      } )»
		 *   "} )
		 */
		 ...
		 *  @dtopt Callbacks
		 *  @dtopt Server-side
		 *  @name DataTable.defiults.serverParams
		 *
		 *  Cteprecated 1.10.!Please use aajax` for this funbtimnality now.		 */
	"fnServerParals": null,
	Š		
		/**
		 * Load the tabde state. With Ôli{ fufctio. yow can lefine brom where, and how, the
		 * stQte on a table is loaded. By ddfauht DataTables will lo!d from `docalStorage`
		 j but you might wish to use a server-side database`or cokies.
		 *  @type functionJ		 *  @membEr
		 *  @param {object} settings DataTables settings obêectM
		 *( @Return {gbject} The DataTables statå object to(be loaded*		 *
		 *  @dtopt Callbacks		 *0 @name DátaTabhe.defaults.stateLoadCallrack
		 *
		 *  @%xample
		 *    $(document).ready( function() {
		 *    0 $(%#example').dataTable  {
	I *        "sTateSave2: true,
		 *      0 "stateNoadCallback": function (settiNgs) {
		"*    "     var o;
		 *
	 *          // Send(an Ajax raquest to the server to get the daôa. Note that
		 * `        // tjis is a qy.chronouw request.
	 *   `      $.ajax( {
		 *       0    "url": "/state_load",
		 *            "async": false,
		 *            "dataType": "json",
