...
$date=$_POST['date'];
$reason=$_POST['reason'];
$inumber = mt_rand(999000000, 999999999);
  
$query=mysqli_query($con,"insert into  tbmeter (INumber,Resident,Street,Reference,Landlord,Address,HNumber,Employee,Email,Mobile,NPipe,Installation,Date,Reason) value('$inumber','$resident','$street','$reference','$landlord','$address','$hnumber','$employee','$email','$mobile','$npipe','$installation','$date','$reason')");
if ($query) {
$ret=mysqli_query($con,"select INumber from tbmeter where Email='$email' and  Mobile='$mobile'");
$result=mysqli_fetch_array($ret);
$_SESSION['inumber']=$result['INumber'];
echo "<script>window.location.href='invoice_number.php'</script>";	
}
else
{
...
<h5 style="font-size:20px; color:blue;">Meter Number Registration | P.o Box 570 | 
<script>
function show2(){
...
<table class="table">
<tr class="form4">
<td colspan="3"align="center"style="font-size:20px;">Meter Number Registration</td>
</tr>
</table>
...
<select name="reference" class="form-control text-box single-line" style="height:35px;font-size:16px;" required="true">
<option disabled selected>Select</option>
<?php $query=mysqli_query($con,"select * from tblappointment");
while($row=mysqli_fetch_array($query))
{
?>
<option value="<?php echo $row['AptNumber'];?>"><?php echo $row['AptNumber'];?></option>
<?php } ?> 
</select>
</div></div>
...
<input type="text" name="landlord" class="form-control text-box single-line" style="height:35px;font-size:16px;"required>
</div>

<div class="col-md-4">
<span style="font-size:16px;color:blue;">Home Address </span><span style="color:red">*</span>
<input class="form-control text-box single-line"type="text" name="address" style="height:35px;font-size:16px;"required/>
</div>
...
<input type="text" name="npipe" class="form-control text-box single-line" style="height:35px;font-size:16px;"required>
</div>


<?php
$installation = array("Complete","Incomplete");

?>
<div class="col-md-4">
<span style="font-size:16px;color:blue;">Installation Status </span><span style="color:red">*</span>
<select name="installation"  class="form-control text-box single-line" style="height:35px;font-size:16px;">
<option disabled selected>Select</option>
<?php
